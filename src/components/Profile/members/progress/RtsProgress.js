...
const RtsProgress = (props) => {
    ...
    if (props.rts.vouchers < 9600) {
        ...
        CompanyProgress = Math.floor((CurrentVouchers /
            RankVouchers) * 100)
    ...
    return (
        <div className="mt-2">
            <h4>RTS {props.member.company === 'rts' &&<small className="text-muted">(current company)</small>}</h4>
            <Table>
                <thead>
                    <tr>
                ...
                <tbody>
                    <tr>
                        <td>{Rank}</td>
                        <td>{props.company.rts_rank}/{props.company.total_members}</td>
                        <td><FormattedNumber num={props.rts.vouchers} /></td>
                        <td>$<FormattedNumber num={props.rts.worth} /></td>
