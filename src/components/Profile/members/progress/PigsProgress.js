...
const PigsProgress = (props) => {
    ...
    return (
        <div>
            <h4>PIGS {props.member.company === 'pigs' &&<small className="text-muted">(current company)</small>}</h4>
            <Table>
                <thead>
                    <tr>
                ...
                <tbody>
                    <tr>
                        <td>{Rank}</td>
                        <td>{props.company.pigs_rank}/{props.company.total_members}</td>
                        <td><FormattedNumber num={props.pigs.vouchers} /></td>
                        <td>$<FormattedNumber num={props.pigs.worth} /></td>
                        <td><FormattedNumber num={RequiredVouchers} /></td>
                        {NextRank === 'Max' && <td>Max Rank</td>}
                        {NextRank !== 'Max' && <td>$<FormattedNumber num={Math.ceil(stolenMoney / 10000) * 10000} /></td>}
                        <td>{NextRank}</td>
                    </tr>
                </tbody>
            </Table>
