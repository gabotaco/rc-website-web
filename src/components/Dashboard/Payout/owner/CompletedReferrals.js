    ...
    return (
        <Query query={queries.GET_COMPLETED_REFERRALS}>
        {
            ({loading, error, data}) => {
                    ...
                    unpaid: (unpaid, i) => {
                        return (
                            <tr key={i}>
                                ...
                                <td>$<FormattedNumber num={referralData.moneyOwned[unpaid.in_game_id].unpaid} /></td>
                                <td><GetDetailsButton referred_id={unpaid.in_game_id} paid={"0"} data={unpaid} /></td>
                                <td><PayReferralButton referred_id={unpaid.in_game_id} name={unpaid.in_game_name} /></td>
                            </tr>
                        )
                    },
                    paid: (paid, i) => {
                        return (
                            <tr key={i}>
                                ...
                                <td>$<FormattedNumber num={referralData.moneyOwned[paid.in_game_id].paid} /></td>
                                <td><GetDetailsButton referred_id={paid.in_game_id} paid={"1"} data={paid} /></td>
                            </tr>
                        )
                    },
                    both: (both, i) => {
                        return (
                            <tr key={i}>
                                ...
                                <td>$<FormattedNumber num={referralData.moneyOwned[both.in_game_id].unpaid + referralData.moneyOwned[both.in_game_id].paid} /></td>
                                <td><GetDetailsButton referred_id={both.in_game_id} paid={"both"}  data={both} /></td>
                            </tr>
                        )
                    }
                }
                ...
                return (
                    <FilterableTables filters={['Unpaid', 'Paid', 'Both']}
                        tables={[
                            {
                                filter: 'Unpaid',
