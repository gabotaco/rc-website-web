...
const MemberDropdown = (props) => {
    ...
    return (
        <React.Fragment>
            <SearchableDropdown title={selectedMember ? selectedMember.in_game_name : 'Select'} placeholder="Search (Discord ID, In game ID, In game name)" data={loading ? 'LOADING' : (error ? 'ERROR LOADING MEMBERS' : data.getCurrentEmployees.map(makeData))} onSelected={memberSelected} isLoading={loading} />
            <Input valid={!!selectedMember} invalid={!selectedMember} style={{display: 'none'}} />
            <FormFeedback valid>
                Looks good!
            </FormFeedback>
            <FormFeedback>
