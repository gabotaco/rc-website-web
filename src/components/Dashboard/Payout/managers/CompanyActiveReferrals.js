            ...
            ({loading, error, data, refetch}) => {
                ...
                const formatter = (applicant, key) => {
                    return <tr key={key}>
                        ...
                        <td>{applicant.re_in_game_id} {props.owner && <ChangeInGameIDButton refetch={refetch} employee_name={applicant.employee_name} game_id={applicant.re_in_game_id} app_id={applicant.app_id} />}</td>
                        <td>{applicant.re_discord_id}</td>
                    </tr>
                }

                return (
                    <CustomTable config={config} headers={headers} data={applicants} format={formatter} />
                )
            }
        }
        </Query>
