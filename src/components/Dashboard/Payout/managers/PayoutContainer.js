...
const PayoutContainer = () => {
	...
	function getTotalRtsVouchers() {
		return (
			parseInt(rtsHeavyVouchers) +
			parseInt(rtsVouchers) +
			parseInt(rtsAviatorVouchers)
		);
	}
	...
	function handleSubmit() {
		...
		Api.calculatePayout(member.id, voucherNum, company)
			.then(response => {
				setLoading(false);
	...
	function handleConfirm() {
		...
		Api.confirmPayout(response.pin)
			.then(() => {
				setLoading(false);
				setAlert(true);
				toggle();
		...
		<Form noValidate autoComplete="off">
			...
			<Row>
				<Col>
					<FormGroup>
						...
						<FormFeedback>Please select a company.</FormFeedback>
					</FormGroup>
				</Col>
				<Col>
					<FormGroup>
			...
			{company === 'pigs' && (
				<FormGroup>
					...
					<Input
						valid={!!pigsVouchers && pigsVouchers !== 0}
						invalid={!pigsVouchers || pigsVouchers === 0}
						type="number"
						placeholder="Voucher Number"
						value={pigsVouchers}
						onChange={ev => setPigsVouchers(ev.target.value)}
						required></Input>
					<FormFeedback valid>Looks good!</FormFeedback>
					<FormFeedback>Please specify a voucher amount.</FormFeedback>
				</FormGroup>
			)}
			{company === 'rts' && (
				<div>
					<FormGroup>
						...
						<Input
							valid={!!rtsHeavyVouchers && rtsHeavyVouchers !== 0}
							invalid={
								(!rtsHeavyVouchers || rtsHeavyVouchers === 0) &&
								getTotalRtsVouchers() === 0
							}
							type="number"
							placeholder="Voucher Number"
							value={rtsHeavyVouchers}
							onChange={ev => setRtsHeavyVouchers(ev.target.value)}
							required
						/>
						<FormFeedback valid>Looks good!</FormFeedback>
						<FormFeedback>Please specify a voucher amount.</FormFeedback>
					...
					<FormGroup>
						...
						<Input
							valid={!!rtsVouchers && rtsVouchers !== 0}
							invalid={
								(!rtsVouchers || rtsVouchers === 0) &&
								getTotalRtsVouchers() === 0
							}
							type="number"
							placeholder="Voucher Number"
							value={rtsVouchers}
							onChange={ev => setRtsVouchers(ev.target.value)}
							required
						/>
						<FormFeedback valid>Looks good!</FormFeedback>
						<FormFeedback>Please specify a voucher amount.</FormFeedback>
					...
					<FormGroup>
						...
						<Input
							valid={!!rtsAviatorVouchers && rtsAviatorVouchers !== 0}
							invalid={
								(!rtsAviatorVouchers || rtsAviatorVouchers === 0) &&
								getTotalRtsVouchers() === 0
							}
							type="number"
							placeholder="Voucher Number"
							value={rtsAviatorVouchers}
							onChange={ev => setRtsAviatorVouchers(ev.target.value)}
							required
						/>
						<FormFeedback valid>Looks good!</FormFeedback>
						<FormFeedback>Please specify a voucher amount.</FormFeedback>
			...
			<Button
				color="primary"
				disabled={!validForm() || modal || loading}
				onClick={handleSubmit}>
				{modal || loading ? <LoadingIcon /> : 'Submit'}
			</Button>
			{member && response && (
				<Modal fade isOpen={modal} toggle={toggle} backdrop={'static'}>
					<ModalHeader toggle={toggle}>
						Payout Summary for {member.in_game_name} ({member.in_game_id})
					</ModalHeader>
					<ModalBody>
						<b>Progress</b>:{' '}
						<Progress multi>
							...
							<Progress
								...
								value={
									response.do_rank
										? 100
										: response.newProgress - response.oldProgress
								}
							/>
						</Progress>
						...
						<b>Rank up</b>?{' '}
						{response.do_rank ? (
							<span className="text-success">{response.do_rank}</span>
						) : (
						...
						<CopyTextButton
							...
							color="info"
							className="mt-2"
							text={`Your payout will be $${formatNumber(response.money)}.`}
							label="Copy message"
						/>
					</ModalBody>
					<ModalFooter>
