...
const CompanyMembersFilter = (props) => {
    ...
    return (
        <ButtonGroup className="mb-2">
            {props.perms > 2 && <Button color="primary" active={manager} onClick={managerOnClick}>Manager</Button>}
            <Button color="rts" active={rts} onClick={() => setRts(!rts)} >RTS</Button>
            <Button color="pigs" active={pigs} onClick={() => setPigs(!pigs)} >PIGS</Button>
            <Button color="yeet" active={fired} onClick={() => setFired(!fired)} >YEETED</Button>
        </ButtonGroup>
    )
}
