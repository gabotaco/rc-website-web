...
const VehicleMods = () => {
	...
	function randomMod() {
		const modString = `{"mods": {"1": ${RandomNumber(
			-1,
			2
		)}, "2": ${RandomNumber(-1, 2)}, "3": ${RandomNumber(
			-1,
			2
		)}, "4": ${RandomNumber(-1, 2)}, "5": ${RandomNumber(
			-1,
			1
		)}, "6": ${RandomNumber(-1, 1)}, "7": ${RandomNumber(
			-1,
			1
		)}, "8": ${RandomNumber(-1, 1)}, "9": ${RandomNumber(
			-1,
			1
		)}, "10": ${RandomNumber(
			-1,
			...
			0,
			20
		)}, "25": ${RandomNumber(0, 5)}, "26": ${RandomNumber(
			0,
			15
		)}, "27": ${RandomNumber(0, 4)}, "28": ${RandomNumber(
			0,
			44
		)}, "29": -1, "30": ${RandomNumber(
			0,
			13
		)}, "31": -1, "32": -1, "33": ${RandomNumber(0, 15)}, "34": ${RandomNumber(
			0,
			14
		)}, "35": ${RandomNumber(0, 21)}, "36": ${RandomNumber(
			0,
			1
		)}, "37": ${RandomNumber(0, 6)}, "38": -1, "39": ${RandomNumber(
			0,
			3
		)}, "40": ${RandomNumber(
			0,
		...
		Api.getBennysList()
			.then(response => {
				...
				response.forEach(vehicle => {
					if (usedVehicles.filter(value => value.inner === vehicle).length > 0)
						return;
					usedVehicles.push({
						inner: vehicle,
	...
	function carSelected(vehicle) {
		setIsLoading(true);
		Api.searchBennys(vehicle.inner)
			.then(response => {
