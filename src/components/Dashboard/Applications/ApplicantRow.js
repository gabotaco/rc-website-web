...
import { Button, Modal, ModalHeader, ModalBody, ModalFooter, Form, FormGroup, FormFeedback, Input, Label } from 'reactstrap';
...
const ApplicantRow = ({applicant}) => {
    ...
    const [newStatus, setNewStatus] = useState(app.status_info === null ? "" : app.status_info.replace(/.*?:\s/, ""))
    ...
    function handleDetailsClick() {
        setLoading(true)
        Api.getApplicantDetails(app.id).then((response) => {
            setLoading(false);
            setApp(response)
            toggle();
        }).catch((err) => {
    ...
    function handleContactClick() {
        ...
        SET_APPLICANT_CONTACTED({
            ...
            setApp({...app, status: 'Contacted', status_info: response.data.setApplicantContacted})
        }).catch((err) => {
            console.error(err); 
    ...
    function handleNewStatusClick() {
        ...
        UPDATE_APPLICANT_STATUS_INFO({
            ...
            setApp({...app, status_info: response.data.updateApplicantStatusInfo})
        }).catch((err) => {
            console.error(err);
    ...
    function handleHireClick() {
        ...
        Api.hire(app.company, app.in_game_name, app.in_game_id, app.discord_id, app.id).then(() => {
            setHireStatus('DONE')
    ...
    return (
        <tr key={app.id}>
            ...
            <td>{app.status_info ? (app.status === 'Contacted' ? app.status_info : `${app.status} (${app.status_info})`) : app.status}</td>
