...
import { Container, Row, Col, Navbar, NavbarToggler, Collapse, Nav, NavItem, Form, Input, Progress } from 'reactstrap';
...
const HomeScreen = (props) => {
    ...
    const [playerListLevel, setPlayerListLevel] = useState(<LoadingIcon inline />)
    ...
    const loadingSkill = (skillName) => <Progress multi style={Style.skillBarContainer} className="my-1">
    <Progress bar value={0} min={0} max={100} style={{backgroundColor: '#0063ca'}}>
        <span className="text-white font-weight-bold" style={Style.skillText}>{skillName} | Level <LoadingIcon inline sizeClass={'glimpsicon-16'} /> / <LoadingIcon inline sizeClass={'glimpsicon-16'} /> <i>(<LoadingIcon inline sizeClass={'glimpsicon-16'} /> EXP)</i></span>
    </Progress>
</Progress>
    ...
    const [completionist, setCompletionist] = useState(loadingSkill('Completionist'))
    ...
    function getData(gameId) {
        ...
        Api.getTycoonData(gameId).then((response) => {
            ...
            if (!response.data.gaptitudes) response.data.gaptitudes = {
                "trucking": {
                    "garbage": 10,
                    "postop": 10,
                    "mechanic": 10,
            ...
            if (company) {
                ...
                if (response.data.licenses && hasCooldown(response.data.licenses.corp_cooldown)) {
            ...
            const overallXp = parseFloat(response.data.gaptitudes.physical.strength || 10) + parseFloat(response.data.gaptitudes.piloting.piloting || 10) + parseFloat(response.data.gaptitudes.train.bus || 10) + parseFloat(response.data.gaptitudes.business.business || 10) + parseFloat(response.data.gaptitudes.piloting.cargos || 10) + parseFloat(response.data.gaptitudes.train.train || 10) + parseFloat(response.data.gaptitudes.ems.ems || 10) + parseFloat(response.data.gaptitudes.ems.fire || 10) + parseFloat(response.data.gaptitudes.farming.farming || 10) + parseFloat(response.data.gaptitudes.farming.fishing || 10) + parseFloat(response.data.gaptitudes.casino.casino || 10) + parseFloat(response.data.gaptitudes.trucking.garbage || 10) + parseFloat(response.data.gaptitudes.piloting.heli || 10) + parseFloat(response.data.gaptitudes.hunting.skill || 10) + parseFloat(response.data.gaptitudes.trucking.mechanic || 10) + parseFloat(response.data.gaptitudes.farming.mining || 10) + parseFloat(response.data.gaptitudes.player.player || 10) + parseFloat(response.data.gaptitudes.trucking.postop || 10) + parseFloat(response.data.gaptitudes.player.racing || 10) + parseFloat(response.data.gaptitudes.trucking.trucking || 10)
            ...
            let storageUsed = 0;
            let capacity = calculateLevel(response.data.gaptitudes.physical.strength) * 10;
            if (hasPremium) {
                capacity *= 1.15
            }
            ...
            setBonus(response.data.licenses && !!response.data.licenses.faq_bonus_exp_5)
            setLinked(response.data.licenses && !!response.data.groups.discord_linked)
            setVoice(response.data.licenses && !!response.data.licenses.discord_voice)

            setBusiness(addSkill(response.data.gaptitudes_v.business.business, 100, "Business"))
            setGambling(addSkill(response.data.gaptitudes_v.casino.casino, 100, "Gambling"))
            setEms(addSkill(response.data.gaptitudes_v.ems.ems, 100, "EMS"))
            setFire(addSkill(response.data.gaptitudes_v.ems.fire, 100, "Fire"))
            setFarming(addSkill(response.data.gaptitudes_v.farming.farming, 100, "Farming"))
            setFishing(addSkill(response.data.gaptitudes_v.farming.fishing, 100, "Fishing"))
            setMining(addSkill(response.data.gaptitudes_v.farming.mining, 100, "Mining"))
            setHunting(addSkill(response.data.gaptitudes_v.hunting.skill, 100, "Hunting"))
            setStrength(addSkill(response.data.gaptitudes_v.physical.strength, 30, "Strength"))
            setCargo(addSkill(response.data.gaptitudes_v.piloting.cargos, 100, "Cargo Pilot"))
            setHeli(addSkill(response.data.gaptitudes_v.piloting.heli, 100, "Helicopter Pilot"))
            setAirline(addSkill(response.data.gaptitudes_v.piloting.piloting, 100, "Airline Pilot"))
            setPlayer(addSkill(response.data.gaptitudes_v.player.player, 100, "Player"))
            setRacing(addSkill(response.data.gaptitudes_v.player.racing, 100, "Racing"))
            setBus(addSkill(response.data.gaptitudes_v.train.bus, 100, "Bus Driver"))
            setConductor(addSkill(response.data.gaptitudes_v.train.train, 100, "Conductor"))
            setGarbage(addSkill(response.data.gaptitudes_v.trucking.garbage, 100, "Garbage Collections"))
            setMechanic(addSkill(response.data.gaptitudes_v.trucking.mechanic, 100, "Mechanic"))
            setPostOp(addSkill(response.data.gaptitudes_v.trucking.postop, 100, "PostOP"))
            setTrucking(addSkill(response.data.gaptitudes_v.trucking.trucking, 100, "Trucking"))
    ...
    function addSkill(xp, maxLevel, skillName) {
        ...
        return <Progress multi style={Style.skillBarContainer} className="my-1">
                <Progress bar value={currentLevel >= maxLevel ? 100 : lvlPercent} min={0} max={100} style={{backgroundColor: currentLevel >= maxLevel ? '#b39700' : '#0063ca'}}>
                    <span className="text-white font-weight-bold" style={Style.skillText}>{skillName} | Level <FormattedNumber num={currentLevel} /> / <FormattedNumber num={maxLevel} /> <i>(<FormattedNumber num={xp}/> EXP)</i></span>
                </Progress>
            </Progress>
    }
                            ...
                            <Nav navbar>
                                ...
                                <NavItem>
                                    ...
                                    <Progress multi style={Style.healthBarContainer}>
                                        <Progress bar value={health} min={0} max={100} style={Style.healthBar} />
                                    </Progress>
                                    <hr className="my-2" />
                                </NavItem>
                                {/* <NavItem>
                                ...
                                <NavItem>
                                    ...
                                    <Progress multi style={Style.capacityBarContainer}>
                                        <Progress bar value={capacity} min={0} max={100} style={Style.capacityBar} />
                                    </Progress>
                                    <hr className="my-2" />
                                </NavItem>
                                <NavItem>
                                ...
                                <NavItem>
                                    <p>Premium: <span className={premium === 'LOADING' ? null : (premium ? 'text-success' : 'text-danger')}>{premium === 'LOADING' ? <LoadingIcon inline /> : (premium ? 'Yes' : 'No')}</span></p>
                                </NavItem>
                                <NavItem>
                                    <p>Double XP: <span className={dxp === 'LOADING' ? null : (dxp ? 'text-success' : 'text-danger')}>{dxp === 'LOADING' ? <LoadingIcon inline /> : (dxp ? 'Yes' : 'No')}</span></p>
                                </NavItem>
                                <NavItem>
                                    <p>15% Boost: <span className={boost === 'LOADING' ? null : (boost ? 'text-success' : 'text-danger')}>{boost === 'LOADING' ? <LoadingIcon inline /> : (boost ? 'Yes' : 'No')}</span></p>
                                </NavItem>
                                <NavItem>
                                    <p>Faction Bonus: <span className={bonus === 'LOADING' ? null : (bonus ? 'text-success' : 'text-danger')}>{bonus === 'LOADING' ? <LoadingIcon inline /> : (bonus ? 'Yes' : 'No')}</span></p>
                                </NavItem>
                                <NavItem>
                                    <p>Discord Linked: <span className={linked === 'LOADING' ? null : (linked ? 'text-success' : 'text-danger')}>{linked === 'LOADING' ? <LoadingIcon inline /> : (linked ? 'Yes' : 'No')}</span></p>
                                </NavItem>
                                <NavItem>
                                    <p>Discord Voice: <span className={voice === 'LOADING' ? null : (voice ? 'text-success' : 'text-danger')}>{voice === 'LOADING' ? <LoadingIcon inline /> : (voice ? 'Yes' : 'No')}</span></p>
                                </NavItem>
                            </Nav>
                        </Collapse>
                    </Navbar>
