...
const ApplyScreen = () => {
	...
	useEffect(() => {
		...
		if (
			authorizedUser.permission > 1 ||
			authorizedUser.company === 'rts' ||
			authorizedUser.company === 'pigs'
		) {
			setisHired(true);
							...
							{invalidDiscord ? (
								rehireCompany === 'rts' ? (
									<h5>
										...
										<br />
										Go to the next slide after joining.
									</h5>
								) : (
									<h5>
										You must join{' '}
								...
								<h5>
									We have you as {authorizedUser.in_game_name}#
									{authorizedUser.in_game_id}. If this is not correct, please
									check with a manager to get this changed.
									<br />
									Your Company:
									<ButtonGroup>
										<Button
											color="rts"
											active={rehireCompany === 'rts'}
											onClick={() => setRehireCompany('rts')}>
											RTS
										</Button>
										<Button
											color="pigs"
											active={rehireCompany === 'pigs'}
											onClick={() => setRehireCompany('pigs')}>
											PIGS
										</Button>
									</ButtonGroup>
								</h5>
								...
								<h5>
									...
									check with a manager to get this changed.
									<br />
									Unfortunately you're either currently in a company or no
									longer meet the minimum requirements (level 30 strength). If
									this is an error let a manager know. Otherwise, check back
									later.
								</h5>
							)}
						</React.Fragment>
					...
					message: (
						<React.Fragment>
							{rehired ? (
								rehireCompany === 'rts' ? (
									<h5>
										Alright, you're hired. Type .roles in any channel of the
										PIGS and RTS Discord and let a manager know you need your
										roles in-game again.
										<br />
										<a
								...
								<h5>
									<LoadingIcon inline /> Hiring... Please wait.
								</h5>
							)}
						</React.Fragment>
					),
			...
			Api.getTycoonData(authorizedUser.in_game_id)
				.then(res => {
					...
					if (!res.data.gaptitudes_v)
						res.data.gaptitudes_v = res.data.gaptitudes;
			...
			setItems([
				{
					message: (
						<h5>
							You're seeing this page because you're back, and you didn't leave
							on a good note.
						</h5>
					),
				},
				{
					message: (
						<h5>
							We have you as {authorizedUser.in_game_name}#
							{authorizedUser.in_game_id}. If this is not correct, please check
							with a manager to get this changed.
							<br />
							Your Company:
							<ButtonGroup>
								<Button
			...
			body: (
				<Form noValidate>
					<FormGroup>
						...
						<Input
							valid={!!inGameName && /[a-zA-Z0-9]*/.test(inGameName)}
							invalid={!inGameName || !/[a-zA-Z0-9]*/.test(inGameName)}
							type="text"
						...
						<FormFeedback>Please specify an in-game name.</FormFeedback>
					</FormGroup>
					<FormGroup>
						<Label>In-Game ID#</Label>
						<Input
							valid={!!inGameId && !!parseInt(inGameId) && !inGameIdErr}
							invalid={!inGameId || !parseInt(inGameId) || inGameIdErr}
							type="number"
							placeholder="Your answer"
							required
							value={inGameId}
							onChange={ev => setInGameId(parseInt(ev.target.value))}
						/>
						<FormFeedback valid>Looks good!</FormFeedback>
						<FormFeedback>
							{inGameIdErr
								? 'Player not found.'
								: 'Please specify an in-game id.'}
						</FormFeedback>
					</FormGroup>
					<FormGroup>
						<Label>
							Were you referred by someone? If so, enter their in-game ID number
							below, otherwise skip ahead. (ex., 81915) YOU CAN ONLY REFFER ONE
							PERSON
						</Label>
						<Input
							valid={!!referredId && !!parseInt(referredId)}
							type="number"
							placeholder="Your answer"
							value={referredId}
							onChange={ev => setReferredId(parseInt(ev.target.value))}
						/>
						<FormFeedback valid>Looks good!</FormFeedback>
					</FormGroup>
				</Form>
				...
				<Form noValidate>
					<FormGroup>
						...
						<Input
							...
							<option value="No, you don't understand. I love Tycoon.">
								No, you don't understand. I love Tycoon.
							</option>
						</Input>
						<FormFeedback valid>Looks good!</FormFeedback>
						<FormFeedback>Please answer how long you play.</FormFeedback>
					</FormGroup>
					<FormGroup>
						...
						<FormGroup check>
							<Input
								...
								type="radio"
								valid={!!company && !notInDiscord}
								invalid={!company || notInDiscord}
							/>
							<Label check>RTS</Label>
							<FormText>
								RTS delivers vehicles. A person can spawn randomized sports cars
								or spawn randomized aircraft and drive/pilot them to their
								destinations.
							</FormText>
						</FormGroup>
						<FormGroup check>
							...
							<FormText>
								PIGS perform heists. These missions can be performed solo but
								you're paid FAR better for doing jobs in a group.
							</FormText>
							<FormFeedback valid>Looks good!</FormFeedback>
							<FormFeedback>
								{notInDiscord ? (
		...
		},
		{
			...
			body: (
				<Form noValidate>
					<FormGroup>
						<Label>
							What country or region are you from? If nothing else, what time
							zone are you in?
						</Label>
						<Input
							value={country}
							...
							valid={!!country && /[a-zA-Z0-9]*/.test(country)}
							invalid={!country || !/[a-zA-Z0-9]*/.test(country)}
							type="text"
							pattern="[a-zA-Z0-9]*"
							placeholder="Your answer"
							required
						/>
						<FormFeedback valid>Looks good!</FormFeedback>
						<FormFeedback>Please specify an answer.</FormFeedback>
					</FormGroup>
					<FormGroup>
						<Label>
	...
	if (
		activeIndex === 2 &&
		!authorizedUser.welcome &&
		authorizedUser.company === 'fired'
	) {
		Api.hire(
			rehireCompany,
			authorizedUser.in_game_name,
			...
			Api.apply(
				...
				.catch(err => {
					...
					if (err.error === 'Missing fields in application') {
						alert(
							"Failed to send one of your responses to the server. Make sure you didn't input any weird characters (emojis) or new lines"
						);
					} else if (err.error === 'You already have a pending application') {
						alert(
							'You already have a pending applcation, this one will be disregarded'
						);
						document.location.reload();
					} else if (
						err.error ===
						'There was a problem adding your application to the database'
					) {
