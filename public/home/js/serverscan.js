    ...
    .then((res) => {
        ...
        for (let i = 0; i < res.players.length; i++) {
            ...
            if (playerMarkers[res.players[i][2]] === undefined) {
                ...
                } else if (res.players[i][5]["group"] == "rts_job" || res.players[i][5]["group"] == "rts_job_air" || res.players[i][5]["group"] == "rts_professional") {
                ...
                playerMarkers[res.players[i][2]] = L.marker([
                    ...
                    .bindTooltip(generateTag(res.players[i][5]["group"]) + res.players[i][0] + ` {${checkbox[2]}}`, {
                        permanent: true,
                        interactive: true,
                        offset: [0, -5],
                        opacity: 0.8,
...
function parsePlayerInfo(data, checkbox) {
    return `<div class="markerHead" data-ingameid="${data[2]}">Player Info</div>
            ...
            <b>Company:</b> ${memberDetails[data[2]].Company.toUpperCase()} ${(managerIDs.includes(data[2]) ? "(Manager)" : memberDetails[data[2]].Company != "Applicant" ? `(${memberDetails[data[2]].Rank})` : "")}<hr>
            <b>Job:</b> ${data[5].name || "N/A"}<hr>
            ${(data[4]["vehicle_label"] === "NULL"? 
                            "" : 
                            `<b>Vehicle</b>: ${data[4]["vehicle_name"]} (${vehicle_classes[data[4]["vehicle_class"]]})<hr>`)}
            <b>${checkbox[1]}</b> <a href="fivem://connect/${checkbox[0]}" title="Join: ${checkbox[0]}">JOIN</a>`
}